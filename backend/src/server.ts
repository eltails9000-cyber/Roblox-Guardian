...
function getGuardianSession(request: express.Request) {
...
app.get('/api/health', (_request, response) => {
  return response.json({
    ok: true,
    robloxConfigured: isRobloxConfigured
  });
});
...
app.get('/api/oauth/callback', async (request, response) => {
  ...
  try {
    ...
    saveConnection(guardianUser, {
      profile: result.profile,
      scopes: [
        ...(
          result.tokens.scope?.split(' ') ??
          ['openid', 'profile']
        )
      ],
    ...
    // HTTPS is required when SameSite=None is used.
...
app.get('/api/me', (request, response) => {
  const connection = isAuthenticated(request)
    ? getConnection(guardianUser)
    : undefined;
...
app.post('/api/oauth/revoke', async (request, response) => {
  if (!isAuthenticated(request)) {
    return response.status(401).json({
      code: 'UNAUTHORIZED',
      message: 'Guardian session required.'
    });
  }

  const connection = getConnection(guardianUser);
...
app.get('/api/sessions', (request, response) => {
  ...
  return response.json({
    sessions: listSessions(guardianUser)
  });
});
...
app.delete('/api/sessions', (request, response) => {
  if (!isAuthenticated(request)) {
    return response.status(401).json({
      code: 'UNAUTHORIZED',
      message: 'Guardian session required.'
  ...
  deleteAllSessions(guardianUser);
...
app.get('/api/activity', (request, response) => {
  ...
  return response.json({
    events: listEvents(guardianUser)
  });
});
